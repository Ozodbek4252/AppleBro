<div>
  ...
  ?>
  <span wire:click="addToWishlist({{ $product->id }})" class="new-item__fav product-item__fav_{{ $product->id }}"
    onclick="favourite('{{ $product->id }}', '{{ $product->main_photo_path }}', '{{ $product->main_photo }}', '{{ $product->name }}')"
    @auth @if ($wishlist) style='background-color: #FF1E1E;' @endif @endauth>
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path @auth @if ($wishlist) fill="#fff" @endif @endauth fill-rule="evenodd" clip-rule="evenodd"
