          ...
          <div class="product-info__choose">
            @foreach ($optionArr[$key] as $option)
              <label for="product-color1-{{ $option['id'] }}">
                ...
                <input type="radio"
                  @if (in_array($option['id'], $selectedOption)) @if ($option['name'] == 'Color') @if ($change_color) checked @endif
                @else checked @endif
            @endif>
            <span>{{ $option['value'] }}</span>
            @if ($option['name'] == 'Color')
              <div class="color" style="border-radius: 50%;">
                <img src="/{{ $option['photo_path'] }}/{{ $option['photo'] }}" alt="img"
  ...
  <div class="product-advantages">
    <div class="product-advantages__item">
      ...
      <div class="product-advantages__text">
        {{ __('home.Доставляем максимально быстро.') }} {{ __('home.По всей') }} UZB.
        {{ __('home.Бесплатно от') }}
        10 000 $
      </div>
    </div>
    <div class="product-advantages__item">
