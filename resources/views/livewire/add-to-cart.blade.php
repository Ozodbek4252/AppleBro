<div>
  <span wire:click="addToCart({{$product->id}})" id="{{$product->id}}" class="new-item__basket product-item__fav_{{ $product->id }}" 
      onclick="addToCart('{{ $product->id }}', '{{ $product->main_photo_path }}', '{{ $product->main_photo }}', '{{ $product->name }}')"
...
<script>
    function addToCart(id, path, photo, name) {
      let response = $.ajax({
        type: "GET",
        url: '/carts/' + id + '/check',
        async: false
      ...
      if (response == 'true') {
        ...
        $('.favourite__btn').html("Savatdan o\'chirildi");
        $('.product-item__fav_' + id).removeClass('favourite-active');
      } else {
        console.log('falsee');
        $('.favourite img').attr('src', path + '/' + photo);
        $('.favourite__name').html(name);
        $('.favourite__btn').html("Savatga qo’shildi &nbsp; <a href=\"/cart\">Ko’rish</a>");
      }
    }
  </script>